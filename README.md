...
policy steps. The rod-task camera mount is rotated diagonally so the top-down
grasp pose does not point the sensor straight into the desk.
...
For the final visual demonstration, the red dynamic box follows one continuous
leftward path at approximately `z=1.28 m`, `y=-0.45 m`:
`x=0.78 -> 0.30 -> -0.78` during `10.4--13.0 s`. The RGB-D tracker confirms
the obstacle near the wrist, and the observation-driven safety shield inserts
