...
then tests amplitudes in ascending order including a one-second dwell. In the
latest run, active-base first detection was `11.44 s`, avoidance began at
`12.20 s`, and the search selected an `0.082 m` lateral displacement with only
`-0.0005 m` vertical change; the obstacle contact audit remained exactly zero.
...
continuous top-down-pinch workspace. The measured nominal carry maximum is
now `0.930 m`; the previous physical run reached roughly `1.40 m`.
...
contact steps and zero force, 30 policy steps with all three cameras seeing
the obstacle, and one RGB-D-triggered adaptive avoidance action. Placement uses a 0.10 m XY
...
inserts one adaptive `CONDITIONAL_OBSTACLE_AVOIDANCE` at `t=12.20 s` with a
selected `0.082 m` lateral offset and negligible vertical component before
...
window, and the two cameras overlap for `30` policy steps. During the later
