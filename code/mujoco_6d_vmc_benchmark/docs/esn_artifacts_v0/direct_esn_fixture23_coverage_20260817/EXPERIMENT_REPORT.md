...
## 通宵 + 晨间 campaign：五条替代路线的系统性排除（v15 增补）

`overnight/`、`overnight2/`，含 oracle 蒸馏（3 horizons × {N160/400/1000} × λ{0,50,100,200}，
435 池 1305 次 oracle 标注 episode）、DAgger 取证（5 个 12 轮运行）、oracle-DAgger、
readout 集成、终局泛化矩阵。

### 1. Oracle 直接蒸馏：全面失败（闭环信号必要性）

counterfactual oracle 单步标签的 BC 学生（435 池，全部配置）：

| 配置 | held-out fx3 | gate |
|---|---|---|
| h12 / h24 / h48 × λ=0 | +1.56 / +0.83 / +0.22 | 3-4/8 |
| h12 / h24 / h48 × λ=100 | +0.16 / +0.19 / +0.22 | 8/8 |
| h24 × N400 / N1000 | +1.18±2.47 / +1.75±4.05 | 8/8, 7/8 |
| h24 × λ=50 / λ=200 | +0.24 / +0.15 | 8/8 |

无一超过 Fixed WBC；OOD 上 ±0.004（学生学成"几乎不动"）。**特权 oracle 的单步贪心
标签不能直接蒸馏为闭环策略——oracle 标签需要闭环迭代（教师的行为轨迹）作为桥梁**。
这是「两阶段特权蒸馏」结构的必要性证明。

### 2. DAgger 取证：配方对强起点全面有害（最终定论）

从 coverage-BC 起点第 1 轮即爆（+3.0/+16.1/+31.9，seed 间方差巨大），12 轮后仍
+9~+29；半标签权重版与 pool-BC（435 条）强起点同样崩（第 1 轮 +3.3/+23）。v1 时代的
成功依赖弱起点（3-trace BC）+恰好 3 轮，不可复现于现代起点。

### 3. Oracle-DAgger：稳定但不过线

oracle-BC 起点的 counterfactual DAgger：轮次曲线 +3.29→+1.29（9 轮后平台），
**单调改善且无崩溃**（对照路线 2 的爆炸），但绝对值仍为正（不超过 Fixed WBC）。
oracle 标签下 DAgger 是良性的，只是天花板仍低于行为蒸馏。

### 4. Readout 集成：灾难性否决

8-seed readout 线性平均后：fx0-3 = +16.6~+21.2，no-rod yield 0.183（超标 36 倍）。
ESN 是闭环非线性系统，readout 平均 ≠ 行为平均；集成路线否决（单 seed 选择或
报告 8-seed 统计是正确做法）。

### 5. v3 单模型终局泛化（再确认 + 新增）

v3 双向 seed251：multi-cycle 12.0（8-seed 均值 12.02 一致）；几何 OOD −2.86/−2.38
（强）；**+x 弱碰撞 −0.33——v3 首次在 +x 方向也优于 Fixed WBC**（v1 单向模型为 +1.0）。

### 总图景

通宵五条路线全部为负/确认性结果，**系统性排除了所有替代路径**：特权信息只能经
教师闭环行为传递（路线 1/3）、DAgger 配方脆弱不可用（路线 2）、模型平均非法（路线 4）。
v3 双向 coverage BC 作为 proposed 的地位由排除法进一步巩固——它现在是唯一被证明
可行的从特权信息到可部署策略的完整路径。

## 三指标训练的综合优化（v16 增补）

### 三指标 Pareto（λ_s 是训练中的三指标旋钮）

对 λ 网格（8 seeds）同时测 RMSE / 前冲 / 力矩变化率（`three_metric/pareto_*.json`）：

| λ_s | fx3 RMSE | surge | torque-rate fx3 | 解读 |
|---|---:|---:|---:|---|
| 0 | **15.06** | 0.034 | 435 | 精度最优，安全指标最高 |
| **10** | 16.30 | 0.022 | **324** | **三指标均衡点：torque-rate 降到 VMC 水平（329）** |
| 100 | 16.08 | **0.016** | 324 | 相对 λ10 无额外收益 |

三个安全类指标（jerk 127→86、surge 0.034→0.022、torque-rate 435→324）随 λ 同步改善
——它们同源于动作急剧度，λ_s 一个训练端正则同时压制三者。

### 推荐配置矩阵（最终）

| 配置 | fx3 ΔRMSE | torque-rate | 镜像 | 适用 |
|---|---:|---:|---|---|
| v3 双向 λ=0（accuracy-optimal） | **−2.11** | 435 | 12.70 (8/8) | 精度敏感 |
| 单向 λ=10（safety-balanced） | −0.88 | **324（=VMC）** | 15.89 (8/8) | **唯一同时 fx3 为负 + 安全达 VMC 水平** |
| v3 双向 λ=10 | +0.26 | 324 | 13.67 (8/8) | 双向数据下 λ 伤 held-out 精度（再次确认） |

### 教师 cost 三指标扩展（负结果）

counterfactual cost 新增 torque-rate/surge 项（权重 mild 0.05/0.10、strong 0.20/0.40），
从 legacy bootstrap 重跑 3 轮 DAgger：两个新教师全面退化（fx1-3 RMSE 16.3/25.9/29.8mm，
远差于 v1 教师 9.2/12.0/15.5）。**教师配方保持冻结；三指标的注入点在学生端（λ_s）而非
教师 cost**——与既往发现一致（教师端改动风险高，学生端正则稳健）。

### 训练流程最终形态（三指标综合）

教师（v1 冻结配方，特权 counterfactual DAgger）→ coverage 行为蒸馏（BC，RMSE 主目标）
→ **λ_s 训练端正则（三指标旋钮，一次拟合同时优化精度与安全性）** → 8-seed 统计。
λ_s=10 时安全性（torque-rate/jerk/surge）全部达到手工 VMC 水平，同时保持对 Fixed WBC
的精度优势——训练中三指标综合的完整可操作答案。

## 正结果：Derivative Matching 修复双向正则冲突，建立双向精度-安全前沿（v17）

### 算法（三项 readout 拟合正则的新机制）

1. **Derivative matching**：平滑正则的目标从「动作差分→0」改为「动作差分→**教师的差分**」
   （`fit_readout(..., smoothness_targets=ΔY)`）——学生允许与教师同样快的必要响应，只惩罚
   超出教师的额外抖动；
2. **方向通道减免**（`smoothness_channel_scales`）：y/yaw 通道承载方向切换信号（双向数据
   的任务必要快速变化），减免 90% 惩罚（精确 per-channel normal equations）；
3. **低通教师差分**（`--derivative-lowpass α`）：目标 = EMA(ΔY)，α=1 纯斜率模仿，
   α<1 剥离教师自身高频抖动、保留必要 ramp——单一目标实现两种语义混合。

### 结果（双向 33+1 traces，8 seeds，`dm_grid/`）

| 配置 | fx3 ΔRMSE | jerk3 | torque-rate | 镜像 | no-rod gate |
|---|---:|---:|---:|---|---|
| λ=0（无正则对照） | −2.11 | 119 | 435 | 12.70 | 8/8 |
| 旧 λ_s=10（v16 方案） | **+0.26 失效** | — | 324 | 13.67 | 8/8 |
| **DM w200 + 减免** | **−1.99±0.08** | 123 | 434 | 13.82 (8/8) | 8/8 |
| DM w200 + lp0.5 + 减免 | −0.91 | 122 | 376 | 12.39 (8/8) | 8/8 |
| **DM w1000 + lp0.3 + 减免** | −0.64 | **104** | **325（=VMC 329）** | **12.53 (8/8)** | 8/8 |

### 三个正结果

1. **双向正则冲突被修复**：DM+减免在正则全开的条件下保住 fx3 −1.99（旧 λ 方案 +0.26），
   且救回 no-rod gate（无减免的 DM 6/8 → 减免后 8/8）；
2. **双向安全甜点 dm1000_lp30**：torque-rate 325 = 手工 VMC 水平，jerk 降 18%，
   **镜像 12.53 为全部实验历史最佳**（超过 v3 λ0 的 12.70），fx3 −0.64 仍优于 Fixed WBC，
   全部 gate 8/8——双向数据上首次「精度优于 FW + 安全达 VMC」同时成立；
3. **双向 Pareto 前沿建立**：DM 权重 × 低通系数给出连续可调的精度-安全操作点
   （−1.99/434 → −0.64/325），旧 λ_s 在双向数据上无前沿（任一权重直接失效）。

### 最终配置矩阵（论文）

| 配置 | fx3 | torque-rate | 镜像 | 场景 |
|---|---:|---:|---|---|
| dm200_relief | −1.99 | 434 | 13.82 | 精度优先（正则版） |
| dm1000_lp30_relief | −0.64 | **325** | **12.53** | 安全均衡（双向 SOTA） |
| v3 λ=0 | −2.11 | 435 | 12.70 | 无正则极限 |

## ESN 核心性：记忆必要性的三级证据链（v18 增补）

### 设计：观测退化的梯度实验（ESN vs 无循环随机特征 RF，8 seeds，双向数据）

RF 消融 = 同维度/同输入缩放/同 readout 训练，仅循环权重置零——分离「循环记忆」这一
ESN 定义性特征（`--disable-recurrence`、`--zero-twist-error`、`--zero-joint-velocity`）。

| 观测集 | ESN fx3 | RF fx3 | ESN gate | RF gate | 判定 |
|---|---:|---:|---|---|---|
| 完整 32D（q, qdot, twist, e, ė） | −2.11 | −2.05 | 8/8 | 8/8 | 平——信息充分，记忆冗余 |
| 削 twist 误差（qdot 保留） | −1.91 | −1.94 | 8/8 | 8/8 | 平——qdot 仍可静态推出速度 |
| **再削 qdot（19D：q, twist, e）** | **+10.1±22.8** | **+20.8±14.7** | **1/8** | **0/8 全崩** | **ESN 优势 10.7mm** |

multi-cycle 同步分化：ESN 17.3±12.1 vs RF 31.8±10.4。

### 结论：循环记忆的条件性必要性（条件性正面结果）

1. **速度观测可用时**，柔顺策略是静态反馈可表达的（ESN=RF=MLP 均值）——诚实呈现；
2. **速度观测不可用时**（低成本编码器、减速器打滑、速度估计被弃用的场景），
   接触动态只能从位置误差**历史**推断：reservoir 的循环状态承担积分/滤波功能，
   ESN 保持残存可工作能力（fx2 仍 +2.4，个别 seed 过 gate），RF 全面崩溃
   （0/8，fx2 +19.9mm）——**循环记忆从「可选增强」变为「性能生死线」**；
3. 结合既有证据：CR 设计塌缩（超参免调）+ 种子稳定性（vs MLP ±0.03/±0.75）+
   O(N) 推理 + 记忆的条件必要性 = **ESN 作为核心算法的完整论证**。

### 论文叙事（ESN 核心版）

**Certifiable-by-design reservoir compliance**：固定收缩 reservoir（ρ<1，状态有界）+
线性读出（输出闭式界）+ 特权两阶段蒸馏（教师→coverage→DM 正则）。三个卖点各有实验：
(a) 常规传感下与 MLP 同精度但种子级稳定、超参免调（CR 定理）；(b) 速度观测退化时
唯一可工作的学习控制器（本节）；(c) derivative-matched 正则给出双向精度-安全 Pareto。
蒸馏管线是训练方法，**reservoir 结构是方法核心**——其循环动力学的记忆在退化传感
场景不可替代。
