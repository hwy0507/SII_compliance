...
7. [训练管线：数据采集、标签标定与蒸馏](#7-训练管线数据采集标签标定与蒸馏)
8. [评测协议](#8-评测协议)
9. [实验结果](#9-实验结果)
...
| 数据组成部分 | 来源 | 真机上对应什么 |
|---|---|---|
| **X（32 维观测）** | 从 MuJoCo 状态读出的本体感受量：q, q̇（编码器量）、ξ_nom / e_p / ė（WBC 内部信号） | ✅ 真机全都有（关节编码器 + 控制器内部计算），无需力传感器 |
...
克隆当前物理状态（mj_copyData，不污染真实环境）
    ↓
生成 9 个候选动作：零动作 / 纯减速(0.22) / {0.22,0.45}减速 × {0.20,0.45,0.75}顺冲击法线让位
    ↓
每个候选前向 rollout 8 步（320 ms）真实物理（同一 WBC、同一伺服、同一安全栈）
    ↓
九项代价打分 → argmin → 该步的标签 a*
```

**代价函数**（三项 headline 指标全部内化）：
...
### 7.3 真实数据文件结构（服务器验证，t035_y.npz）

每条轨迹 155 步 × 40 ms，字段分三组：

| 组 | 字段 | 用途 |
|---|---|---|
| **学生输入**（32 维原始量） | joint_position(7), joint_velocity(7), wbc_task_twist(6), wbc_pose_error(6), wbc_twist_error(6) | 训练 X |
| **标签** | bounded_action(7) | 训练 Y（教师选出） |
| 特权诊断（学生永远看不到） | contact_force, contact_normal, signed_distance, contact_duration, contact_seen, ee_position... | 教师标注用 / 调试用 |

一条典型轨迹的标注模式（t035_y，实测）：

```
t=1.00–1.16   F=0 N       |a|=0.000     ← 接近段：教师标签恒零（中性）
t=1.24        F=24 N      |a|=0.003     ← 撞上瞬间：几乎不动（不让冲击恶化）
t=1.32        F=17.6 N    |a|=0.123     ← 挤压中：开始让位
t=1.40–1.72   F=0（棒已撤）|a|≈0.30      ← 恢复段：持续助推回轨迹
```

即学生要学的完整行为时序：**不撞不动 → 撞时不硬顶 → 撤后助推恢复**。
...
MLP 用**同一批轨迹、同一个 `_load_episode` 加载器**训练（§5.3）。
