...
### 5.6 参数合理性 ablation（2026-08-19，8 seeds × 18 配置 × 4 fixtures）

**协议**：与主线完全相同的训练数据（73 条教师轨迹，repeat=4）、相同评测脚本
（`evaluate_direct_esn_post_contact.py`，`--robot fr3 --execution-mode torque_residual
--residual-torque-scale 0.05 --seed 20260817`）、固定 8 个 reservoir seed
{251,137,42,71,512,1009,307,13}。唯一变化的是被 sweep 的参数。共 120 个独立
checkpoint、576 次 rollout 评测。

**协议自检（反伪造的黄金证据）**：ablation 启动时先用主线 checkpoint
`esn_bc_251.npz` 在本协议下重跑 fx3，与 32-seed 实验时存储的 JSON 对账：

```text
重跑 ESN RMSE   = 6.280823669691147
存储 ESN RMSE   = 6.280823669691147   ← 逐位一致
重跑 FW  RMSE   = 24.291280749670726
存储 FW  RMSE   = 24.291280749670726  ← 逐位一致
```

**fx3（held-out）ΔRMSE（负 = 优于 Fixed WBC，mean±std over 8 seeds）**：

| N | ΔRMSE (mm) | ρ | ΔRMSE | τ (s) | ΔRMSE | input_scale | ΔRMSE |
|---|---|---|---|---|---|---|---|
...
![Ablation](ablation_fx3.png)

**四个结论（逐条回应"参数是否乱设"）**：

1. **τ（记忆时间常数）是唯一敏感参数**。τ=0.04 s（= dt，泄漏率 1.0，reservoir
   无记忆）性能塌到 −14.8±3.2，逼近无记忆结构；τ≥0.12 饱和于 −18.1。正面验证
   核心假说：**性能来自 reservoir 的时序记忆**。τ=0.12 取在拐点。
2. **N=160 在容量饱和拐点上**。N≤64 时 held-out 掉 3 mm 且种子方差×4
   （±3.3 vs ±0.8）——小 reservoir 记不住撞击动态的完整模式；N∈[100,250] 饱和，
   160 是饱和区内方差最小的点。Fan Ye 论文对齐规模 N=64 仍有 −15.2±3.5，
   依旧优于 MLP（−9.8）与 VMC（−8.9）。
3. **谱半径与输入尺度在平台区**（ρ∈[0.7,1.1]、ins∈[0.15,1.0] 变化 <1 mm）——
   回声态性质成立时性能对二者不敏感，与 Fan Ye 论文 CR 筛选塌缩结论一致。
4. **默认配置 = 每条曲线的拐点 + 最小方差区**，不是在考试 fixture 上调出的
   峰值（fx3 峰值在 input_scale=1.0 处，未采用）。全部 18 配置 fx3 任务成功率 100%。

数据：`ablation_table.csv`（4 fixtures 全量）、`ablation_summary.json`（逐 seed
明细与成功率）、`ablation_fx3.png`。原始 576 份 JSON 在服务器
`outputs/direct_esn_fixture23_coverage_20260817/ablation/ev/`。
