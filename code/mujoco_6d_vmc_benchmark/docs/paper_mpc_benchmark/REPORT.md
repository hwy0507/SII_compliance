...
> **v4 公平调参终版**：基线给足调参（VMC k 扫描 ×5、MLP 容量/种子 ×6）后的最终结论

## 0.7 v4：基线公平调参后的修正结论（撤回 v3 的"超单一 VMC"表述）

v3 的 MLP/VMC 用的是欠调配置（MLP 默认超参单种子；VMC 统一配置直接沿用 rod 最优
k2.2 未做统一扫描），感谢质疑后补齐：VMC k∈{1.5,1.8,2.2,2.6,3.0} ×10 格、
MLP hidden∈{64,128,256}×seed∈{1,2}（800 epochs）。**撤回 v3 中"ESN 超过单一 VMC"
的结论**——VMC k1.5 与调参后 MLP 同样达到 10/10。

**v4 公平终表**（统一 3% 预算，10 格 = rod4+ball4+board2，均为各自最优配置）：

| 方法 | 总分 | 8格 at-grasp 均值/最差 | 板接触力 | 达 10/10 的配置比例 |
|---|---|---|---|---|
| PaperMPC 裸机 | 5/10 | 24.9 / 29.4 mm | 63.0N | — |
| **ESN（免调参，3 种子）** | **10/10 ×3** | **9.5-10.9 / 14.7-16.2 mm** | **22.9-56.3N** | **3/3 (100%)** |
| MLP（最优 h128s2） | 10/10 | 21.1 / 30.0 mm | 27.6N | 5/6 (83%) |
| VMC（最优 k1.5） | 10/10 | 20.5 / 24.0 mm | 96.3N | 1/5 k 值 (20%) |

**v4 最终结论**：
1. **成功率已饱和**：调参后三种柔顺方法都到 10/10——本基准难度下成功不再区分方法；
2. **ESN 独占精度轴**：at-grasp 均值 9.5-10.9mm，比调优后的 MLP（21.1）和 VMC（20.5）
   **紧 2 倍**；ESN 最差种子 16.2mm 仍优于两者均值；
3. **免调参稳定性**：ESN 3/3 种子零调参全过；MLP 5/6 配置过；VMC 仅 1/5 k 值过
   （k 敏感度最高——选错一个 k 就丢一格）；
4. 板场景力域：ESN s101（22.9N）≈ MLP 最优（27.6N）< 裸机（63.0N）< VMC k1.5（96.3N，
   其最优成功配置反而更"用力"——软弹簧在板上滑得慢、接触时间长）。

## 0.6 v3 记录：木板场景的两层修复（保留存档）
