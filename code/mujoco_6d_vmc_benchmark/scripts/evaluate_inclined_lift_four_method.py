...
def run_one(menagerie: Path, label: str, controller, *, seed: int, tilt: float,
            budget: float, board_y_offset_m: float) -> dict:
    fx = fixture(seed)
...
def main() -> None:
    ...
    for label, controller, budget in methods:
        for tilt in args.tilts:
            for seed in args.seeds:
                if controller is not None and hasattr(controller, "reset"):
                    controller.reset()
                offset = float(np.random.default_rng(seed * 1009 + int(round(tilt * 10))).uniform(-0.008, 0.008))
    ...
    output = {
        ...
        "board_geometry": "MuJoCo lift_board box, 0.18 x 0.05 x 0.008 m, friction 0.15, tilt values recorded per row",
