...
def render_one(menagerie, seed, tilt, label, controller, budget, output, fps):
    import mujoco
    ...
    while not done:
        ...
        if step % stride == 0:
            ...
            draw.text((12, 34), f"inclined-board lift   tilt={tilt:.0f} deg   t={t:.2f} s", fill=(202,218,238), font=small)
    ...
    return {"method": label, "tilt_deg": tilt, "seed": seed, "gif": str(output),
...
def main():
    ...
    p.add_argument("--tilt", type=float, default=40.0); p.add_argument("--fps", type=int, default=25)
    a = p.parse_args()
    ...
    manifest = {"schema_version": 1, "purpose": "physical MuJoCo inclined-board demo", "tilt_deg": a.tilt, "seed": a.seed, "methods": []}
    for label, ctrl, budget, name in methods:
        result = render_one(a.menagerie, a.seed, a.tilt, label, ctrl, budget, a.output_dir / name, a.fps)
