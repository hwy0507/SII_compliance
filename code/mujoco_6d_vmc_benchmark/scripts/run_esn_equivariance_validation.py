#!/usr/bin/env python3
"""Evaluate a deployable symmetry-equivariant ESN variant on a fresh split.

The frozen CEM ESN is compared with an otherwise identical variant that
enables the existing pose-error mirror gate.  The gate is an algorithmic
equivariance constraint: it flips only lateral/yaw yield channels under a
measured reflection of the WBC pose error.  It receives no force, contact,
apparatus, obstacle, timing, or future information.  ESN variant selection
and VMC k/budget selection happen on a new validation split; held-out test
seeds are untouched until both selections finish.
"""
...
from direct_esn_compliance import DirectESNController  # noqa: E402
...
def fixtures_for_seed(seed: int, count: int) -> list[VelocityResidualFixture]:
    return [fixture(np.random.default_rng(np.uint64(seed) * 4099 + index + 1))
...
def load_esn_variant(path: Path, mirror_gate: bool) -> DirectESNController:
    controller = load_controller(path)
    if not isinstance(controller, DirectESNController):
        raise TypeError(f"expected DirectESNController, got {type(controller).__name__}")
    if mirror_gate:
...
def evaluate_esn(menagerie: Path, model_path: Path, budget: float, mirror_gate: bool,
                 seeds: list[int], fixture_count: int, label: str) -> list[dict]:
    rows: list[dict] = []
    for seed in seeds:
        for index, fx in enumerate(fixtures_for_seed(seed, fixture_count)):
            controller = load_esn_variant(model_path, mirror_gate)
            row = run_rollout(menagerie, fx, impactor_kind="cross_contact_hand_proxy",
                              controller=controller, residual_scale=budget, seed=seed,
                              verbose_name=f"{label}/fx{index}")
            row["fixture_index"] = index
            row["mirror_gate_enabled"] = mirror_gate
            rows.append(row)
    return rows
...
def evaluate_vmc(menagerie: Path, k: float, budget: float, seeds: list[int],
    ...
    for seed in seeds:
        for index, fx in enumerate(fixtures_for_seed(seed, fixture_count)):
            row = run_rollout(
                menagerie, fx, impactor_kind="cross_contact_hand_proxy",
...
def main() -> None:
    ...
    for name, gate in (("frozen_esn", False), ("mirror_equivariant_esn", True)):
        rows = evaluate_esn(args.menagerie, args.frozen_esn, args.esn_budget, gate,
                            args.validation_seeds, args.fixture_count, f"{name}_validation")
        summary = aggregate(rows)
        esn_candidates.append({"model": name, "mirror_gate_enabled": gate,
                               "budget": args.esn_budget, "summary": summary})
    ...
    for k in args.vmc_k_values:
        for budget in args.vmc_budgets:
            rows = evaluate_vmc(args.menagerie, k, budget, args.validation_seeds,
                                args.fixture_count, f"vmc_k{k:g}_b{budget:g}_validation")
            summary = aggregate(rows)
    ...
    selected_esn = max(esn_candidates, key=lambda candidate: score(candidate["summary"]))
    ...
    test_esn_rows = evaluate_esn(
        args.menagerie, args.frozen_esn, args.esn_budget,
        selected_esn["mirror_gate_enabled"], args.test_seeds, args.fixture_count,
        "selected_esn_test",
    )
    test_vmc_rows = evaluate_vmc(
        args.menagerie, selected_vmc["k"], selected_vmc["budget"],
        args.test_seeds, args.fixture_count, "selected_vmc_test",
    )
    output = {
        "schema_version": 1,
        "protocol": "fresh_split_esn_mirror_equivariance_selection_then_vmc_selection_then_heldout",
        "status": "confirmatory_esn_algorithm_variant",
        "algorithm": "pose-error mirror-equivariant yield gate; lateral/yaw channels are multiplied by the soft sign of deployable measured pose error",
        "frozen_parent": str(args.frozen_esn),
        ...
        "esn_selection_rule": ["maximize validation task_success rate", "break ties with minimum validation at-grasp error"],
        "vmc_selection_rule": ["maximize validation task_success rate", "break ties with minimum validation at-grasp error"],
        "validation_seeds": args.validation_seeds,
        "test_seeds": args.test_seeds,
        "fixture_count_per_seed": args.fixture_count,
        "fixture_generator": "fixture(seed*4099 + fixture_index + 1)",
        "esn_candidates": esn_candidates,
        ...
        "test_summary": {"esn": aggregate(test_esn_rows), "vmc": aggregate(test_vmc_rows)},
        "test_rows": test_esn_rows + test_vmc_rows,
    }
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(output, indent=2) + "\n")
    print(json.dumps({"selected": output["selected"], "test_summary": output["test_summary"]}, indent=2), flush=True)
