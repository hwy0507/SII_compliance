"""Audited primitive VMC-teacher rollout for source-domain collection.
...
from primitive_contact_scene_v6_20260917 import SceneSpec,extend_xml,task_layout
...
def create_environment(a):
    ...
    if a.scene=="ball":m.opt.timestep=min(a.dt,.0000125)
    ...
    task=BlindTask(initial,goal,rotation,env._hand_id,fingers,fdofs,a.settle_only,extended=a.scene!="corner")
...
def run(a):
    ...
    audit=dict(peak_force_n=0.,all_external_peak_force_n=0.,peak_body_resultant_n=0.,max_penetration_m=0.,
        ...
        first_stage=None,first_stage_name=None,first_payload_held=None,pairs=set(),robot_geoms=set(),
        robot_bodies=set(),force=np.zeros(3),other_contacts=set())
    ...
    for step in range(round(a.duration/RL_DT)):
        ...
        if fixture_triggered and task.index==requested_stage:
            if a.scene=="ball":
                ...
                phase_locked=d.time<fixture_trigger_time+a.push_press+a.push_hold+a.push_retract
    ...
    success=bool(lift>.10 and motion<.02 and np.mean(error[-10:])<.015 and not invalid and task.index==len(task.goals)-1)
    cup_support=False
    if cup>=0:
    ...
    result=dict(scene=asdict(spec),controller="vmc" if policy is None else policy.kind,success=success,
        ...
        real_contact=audit["duration_s"]>(1e-6 if a.scene=="ball" else .001),invalid_physics=invalid,contact=audit,events=events,
        ...
        source_sha256={n:hashlib.sha256(Path(__file__).with_name(n).read_bytes()).hexdigest() for n in [Path(__file__).name,"primitive_contact_scene_v6_20260917.py","contact_memory_vmc_20260916.py"]})
...
if __name__=="__main__":
    p=argparse.ArgumentParser();p.add_argument("--scene",choices=("ball","push","corner","empty"),required=True)
