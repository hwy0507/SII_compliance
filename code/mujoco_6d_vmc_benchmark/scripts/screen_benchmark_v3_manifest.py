#!/usr/bin/env python3
"""Freeze a harder, independent V3 lateral-impact generalization suite.

V3 is deliberately separate from the frozen V2 test set.  It changes impact
height and timing while retaining physically measured force/impulse and full
task-validity gates.  The fixed tapered-VMC selector establishes only whether
the geometry is a valid physical task; it never ranks later controllers.

This first V3 release remains truthful about its scope: it is a harder
*off-nominal lateral* generalization set, not yet arbitrary 3-D impact.
"""
...
def main() -> None:
    ...
    parser.add_argument("--start-times", type=float, nargs="+", default=[0.980, 1.160])
    parser.add_argument("--strokes", type=float, nargs="+", default=[0.170, 0.175])
    parser.add_argument("--heights", type=float, nargs="+", default=[0.525, 0.555])
    args = parser.parse_args()
    if not args.start_times or not args.strokes or not args.heights:
        raise ValueError("V3 candidate axes must be non-empty")
    ...
    for side in ("negative_y", "positive_y"):
        for timing_index, start in enumerate(args.start_times):
            for stroke_index, stroke in enumerate(args.strokes):
                for height_index, height in enumerate(args.heights):
                    fixture_id = f"v3_{side}_t{timing_index}_s{stroke_index}_h{height_index}"
                    ...
                    common = dict(
                        ...
                        rod_height_m=float(height), controller_mode="vmc_taper", rod_approach_side=side,
                    ...
                    candidates.append({
                        ...
                        "height_bin": "lower" if height_index == 0 else "upper",
                        "rod_start_time_s": float(start), "rod_stroke_m": float(stroke), "rod_height_m": float(height), "grasp_time_s": 2.40,
    ...
    for row in candidates:
        ...
        if row["selector_valid"]:
            selected.append({key: row[key] for key in (
                "fixture_id", "rod_approach_side", "timing_bin", "stroke_bin", "height_bin", "realized_impulse_bin",
                "rod_start_time_s", "rod_stroke_m", "rod_height_m", "grasp_time_s",
            )})
    manifest = {
        "schema_version": 3,
        "stage": "frozen off-nominal lateral physical rod generalization benchmark",
        "scope": "harder timing/height lateral impacts; not arbitrary 3-D impacts",
        "relationship_to_v2": "independent V3 candidate geometry; V2 manifest and metrics remain frozen",
        ...
        "candidate_design": {"approach_sides": ["negative_y", "positive_y"], "start_times_s": args.start_times, "strokes_m": args.strokes, "heights_m": args.heights},
