...
/** \brief  Header file for all ikfast c++ files/shared objects.
    ...
    - <b>Common</b> - the abstract classes section that all ikfast share
   regardless of their settings
    - <b>Library Specific</b> - the library-specific definitions, which depends
   on the precision/settings that the library was compiled with

    The defines are as follows, they are also used for the ikfast C++ class:

   - IKFAST_HEADER_COMMON - common classes
   - IKFAST_HAS_LIBRARY - if defined, will include library-specific functions.
   by default this is off
   - IKFAST_CLIBRARY - Define this linking statically or dynamically to get
   correct visibility.
   - IKFAST_NO_MAIN - Remove the ``main`` function, usually used with
   IKFAST_CLIBRARY
   - IKFAST_ASSERT - Define in order to get a custom assert called when NaNs,
   divides by zero, and other invalid conditions are detected.
   - IKFAST_REAL - Use to force a custom real number type for IkReal.
   - IKFAST_NAMESPACE - Enclose all functions and classes in this namespace, the
...
/// if 0x10000000 bit is set, then the iksolver assumes 6D transforms are done
/// without the manipulator offset taken into account (allows to reuse IK when
/// manipulator offset changes)
...
class IkSingleDOFSolutionBase {
 public:
  IkSingleDOFSolutionBase()
      : fmul(0), foffset(0), freeind(-1), maxsolutions(1) {
  ...
  unsigned char maxsolutions;  ///< max possible indices, 0 if controlled by
                               ///< free index or a free joint itself
  unsigned char indices[5];    ///< unique index of the solution used to keep
                               ///< track on what part it came from. sometimes a
                               ///< solution can be repeated for different
  ///< indices. store at least another repeated root
};

/// \brief The discrete solutions are returned in this structure.
///
/// Sometimes the joint axes of the robot can align allowing an infinite number
/// of solutions. Stores all these solutions in the form of free variables that
/// the user has to set when querying the solution. Its prototype is:
template <typename T>
class IkSolutionBase {
 public:
  ...
  /// before a full solution can be returned
  ///
...
class IkSolutionListBase {
 public:
  ...
  /// parameters of the solution that users can freely set. The indices are of
  /// the configuration that the IK solver accepts rather than the entire robot,
  /// ie 0 points to the first value accepted.
  ...
  /// \brief clears all current solutions, note that any memory addresses
  /// returned from \ref GetSolution will be invalidated.
...
class IkFastFunctions {
 public:
  ...
  typedef bool (*ComputeIk2Fn)(const T*, const T*, const T*,
                               IkSolutionListBase<T>&, void*);
...
/** \brief Computes all IK solutions given a end effector coordinates and the
   free joints.

   - ``eetrans`` - 3 translation values. For iktype
   **TranslationXYOrientation3D**, the z-axis is the orientation.
   - ``eerot``
   - For **Transform6D** it is 9 values for the 3x3 rotation matrix.
   - For **Direction3D**, **Ray4D**, and **TranslationDirection5D**, the first 3
   values represent the target direction.
...
/** \brief Similar to ComputeIk except takes OpenRAVE
 * boost::shared_ptr<RobotBase::Manipulator>*
 */
...
/// function is used to double check ik.
...
/// \brief a hash of all the chain values used for double checking that the
/// correct IK is used.
