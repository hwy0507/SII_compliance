...
def load_moveit_yaml_scene(data):
    ...
    for co in data["world"]["collision_objects"]:
        obj = {"name": co["id"]}
        base_pose = (
            identity_matrix() if "pose" not in co else transform_from_yaml(co["pose"])
        )
        primitive, prim_pose = co["primitives"][0], transform_from_yaml(
...
def load_moveit_yaml_request(data, joints):
    ...
    jgn, jgp = zip(
        *[
            (e["joint_name"], e["position"])
            for e in data["goal_constraints"][0]["joint_constraints"]
        ]
    )
...
def test_problem(robot, problem):
    ...
    return vamp_module.validate(start, env) and any(
        vamp_module.validate(goal, env) for goal in goals
    )
