...
def main(
    ...
    hf = vamp.png_to_heightfield(
        maze,
        (0.0, 0.0, 0.0),
        (1.0 / 256 * (2.0 * x), 1.0 / 256 * (2.0 * y), 1.0 / z),
    )
    ...
    settings = vamp.PRMSettings(
        vamp.PRMNeighborParams(vamp.sphere.dimension(), vamp.sphere.space_measure())
    )
    ...
    print(
        f"Created roadmap with {len(roadmap)} nodes in {roadmap.nanoseconds / 1e9} seconds!"
    )
    ...
    if visualize:
        ...
        pb.add_height_map(
            maze,
            maze,
            scale=[1.0 / 256 * (2 * x), 1.0 / 256 * (2 * y), z],
            center=[0.0, 0.0, z / 2],
        )
