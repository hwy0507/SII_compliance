...
namespace vamp::binding
{
    ...
    struct Helper
    {
            ...
            const EnvironmentInput &environment) -> bool
        {
            ...
            // std::cout << "Validate config inside C++ " << copy << std::endl;
            // std::cout << "Is all value smaller than 1? " << (copy <= 1.F).all() << std::endl;
            // std::cout << "Is all value greater than 0? " << (copy >= 0.F).all() << std::endl;
            ...
            const EnvironmentInput &environment) -> bool
        {
            ...
            const Configuration goal_v(goal);
            return vamp::planning::validate_motion<Robot, rake, Robot::resolution>(
                start_v, goal_v, EnvironmentVector(environment));
        }
    ...
    inline auto init_robot(nanobind::module_ &pymodule) -> nanobind::module_
    {
        ...
        submodule.def(
            ...
            "goal"_a,
            "environment"_a = vamp::collision::Environment<float>(),
            "Check if a motion is valid. Returns true if valid.");
