...
AnyPlanningResult = Union[fetch.PlanningResult,]
...
def configure_robot_and_planner_with_kwargs(
    ...
    if planner_name == "rrtc":
        ...
        plan_settings = PRMSettings(
            PRMNeighborParams(robot_module.dimension(), robot_module.space_measure())
        )
        ...
        raise NotImplementedError(
            f"Automatic setup for planner {planner_name} is not implemented yet!"
        )
...
def problem_dict_to_vamp(
    ...
    for obj in problem["box"]:
        if obj["name"] not in ignore_names:
            cuboid = Cuboid(
                obj["position"], obj["orientation_euler_xyz"], obj["half_extents"]
            )
            cuboid.name = obj["name"]
            env.add_cuboid(cuboid)
    ...
    if simplification_result:
        simp_data = {
            "simplification_time": pd.Timedelta(
                nanoseconds=simplification_result.nanoseconds
            ),
