...
def problem_to_pointcloud(problem, n):
    np.random.seed(0)
    return np.vstack(
        [cylinder_to_pc(c, n) for c in problem["cylinder"]]
        + [box_to_pc(box, n) for box in problem["box"]]
    )
...
def problem_dict_to_pointcloud(
    ...
    if robot in ROBOT_MAX_RADII.keys():
        ...
        print(
            f"Max extension radius for {robot} not found. Substituting {filter_cull_radius}m."
        )
