...
def name_to_color(name: str) -> List[float]:
    return [*hsv_to_rgb(string_to_01(name), 0.5, 0.9), 1.0]
...
class PyBulletSimulator:
    ...
    def __init__(self, urdf: str, joints: List[str], visualize: bool = True):
        ...
        if urdf:
            self.urdf = urdf
            with DisableRendering(self.client), RedirectStream(
                sys.stdout
            ), RedirectStream(sys.stderr):
        ...
        if urdf and joints:
            ...
            jt = sorted(
                filter(lambda ji: ji[1] in joints, jtu),
    ...
    def add_capsule(
        ...
        with DisableRendering(self.client):
            ...
            col_shape_id = self.client.createCollisionShape(
                pb.GEOM_CAPSULE, radius=radius, height=length
            )
    ...
    def update_object_position(
        ...
        multibody_id: int,
        position: Position,
        rot_xywz: XYZWQuaternion = [0, 0, 0, 1],
    ):
    ...
    def add_height_map(
        ...
        with DisableRendering(self.client), RedirectStream(sys.stdout), RedirectStream(
            sys.stderr
        ):
            shape_id = self.client.createCollisionShape(
                shapeType=pb.GEOM_HEIGHTFIELD,
                meshScale=scale,
                fileName=str(height_file),
            )

            terrain = self.client.createMultiBody(
                baseCollisionShapeIndex=shape_id, basePosition=center
            )
    ...
    def add_environment_from_problem_dict(
    ...
    def draw_pointcloud(self, pc, lifetime: float = 0.0):
...
Use left/right arrow keys to move through individual states."""
        ...
        while True:
            ...
            if space_code in keys and keys[space_code] & self.client.KEY_WAS_TRIGGERED:
                ...
                not playing
                and left in keys
                and keys[left] & self.client.KEY_WAS_TRIGGERED
            ):
                plan_idx -= 1
                if plan_idx < 0:
                    plan_idx = len(plan) - 1
                ...
                not playing
                and right in keys
                and keys[right] & self.client.KEY_WAS_TRIGGERED
            ):
                plan_idx += 1
